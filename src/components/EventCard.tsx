...
export const EventCard = ({ 
  ...
  type = 'normal', 
  onClick 
}: EventCardProps) => { // Removed React.FC typing to avoid children issues
  ...
  return (
    <Pressable
      ...
      `}
    >
      <View className={`w-[52px] h-[52px] rounded-2xl items-center justify-center shrink-0 ${config.classes}`}>
        {config.icon}
      </View>

      <View className="flex-1 min-w-0">
        <View className="flex-row items-center justify-between mb-1">
          <Text variant="body" weight="bold" className="text-white flex-1 mr-2" numberOfLines={1}>
            {item.title}
          </Text>
          ...
          <View className="shrink-0">
            ...
            ) : ((showUrgency && isProtected && item.priority === 'must') || isProtected) ? (
        ...
        <View className="flex-row items-center flex-wrap gap-x-3 gap-y-1">
           {isLocked ? (
             ...
             <View className="flex-row items-center gap-3">
                ...
                {/* Days Left Logic */}
                {isHardDeadline && !isHardDeadlineMissed && (() => {
        ...
        {/* AI Insight */}
        {showAI && (isProtected || isRisk) && !isHardDeadline && (
           <Text 
             variant="caption" 
             weight="medium"
